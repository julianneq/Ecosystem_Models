...
Tsim = function(H,u,eps) {
  ...
  for(i in 3:(nt-1) )  {
    ...
    # Julie: changed Hvory[i+1] to Hvory[i]
    Hvory[i] = max(Hflux,0.1)
    Wnext = Wt[i] + (ratemat[i,2])*dt - LW*Wt[i]*dt + u*LW*Wt[i-1]
    Wt[i+1] = max(Wnext,1)
    # Julie: changed eps[i] and eps[i-1] to eps[i+1] and eps[i], 
    Gnext = Gt[i] + phi*(Gt[i]-Gt[i-1]) + (ratemat[i,1]*dt) - phi*ratemat[(i-1),1]*dt - 
      Hvory[i]*dt + eps[i+1] - theta*eps[i]
    Gt[i+1] = max(Gnext,1)
  }
  # Julie: added calculation of Hvory[nt]
  Hvory[nt] = H*Qt[nt]*Gt[nt] - (uG+phi)*H*Qt[nt-1]*Gt[nt-1] + uG*phi*H*Qt[nt-2]*Gt[nt-2]
...
tvec = (1:(2^11))
#eps = sigma*rnorm(nt)
#write(eps, file="epsilon_grass.txt", ncolumns=1)
eps = scan("epsilon_grass.txt")
...
# write output to file to compare with Python
write.table(Xmat,"grassland_tseries.txt")
spectra = data.frame(sp.con[[2]],sp.nocon[[2]],sp.white[[2]])
write.table(spectra,"grassland_spectra.txt")
