...
Tsim = function(h,u,eps) {
  ...
  for( i in 3:(nt-1) )   {
    ...
    Ht[i] = h*xt[i]
    gx[i]=Gfun(xt[i]) - Ht[i]
    # Julie: changed eps[i] to eps[i+1]
    xnext=xt[i] + (u+v)*dx1 - u*v*dx2 + gx[i] - (u+v)*gx[i-1] + u*v*gx[i-2] + eps[i+1]
...
#u = 0.8 # Julie changed to 0.6 to be consistent with crash count simulation
u = 0.6
...
# save output to test against Python code
harvest = data.frame(Hsim.con, Hsim.nocon, Hsim.white)
write.table(harvest,"fishHarvest_noise_mgmt.txt")
spectra = data.frame(sp.con[[2]], sp.nocon[[2]], sp.white[[2]])
write.table(spectra, "fishSpectra_noise_mgmt.txt")
