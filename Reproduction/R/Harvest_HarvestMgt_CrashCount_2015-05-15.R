...
# Generate/read in random noise vector to verify Python code
#eps = sigma*rnorm(nt)
#write(eps, file="epsilon2_fish.txt", ncolumns=1)
eps = scan("epsilon2_fish.txt")
...
for(i in 1:nh) {
  h = hvec[i]
  Crash.nocon[i] = CrashCount(h,u,eps)
}
...
for(i in 1:nh) {
  h = hvec[i]
  Crash.con[i] = CrashCount(h,u,eps)
}
...
# write output to test against Python code
crashMatrix = data.frame(T2crash.con, T2crash.nocon)
write.table(crashMatrix,"fishCrash_harvest_mgmt.txt")
