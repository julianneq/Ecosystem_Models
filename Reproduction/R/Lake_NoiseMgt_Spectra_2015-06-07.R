...
Tsim = function(lamda,u,eps) {
  ...
  ratemat = matrix(0,nr=nt-1,ncol=2)
  ...
  for(i in 3:(nt-1) )  {
    ...
    Wnext = Wt[i] + (u+phi)*(Wt[i]-Wt[i-1]) - u*phi*(Wt[i-1]-Wt[i-2]) +
      ...
      (1-(u+phi)+u*phi)*lamda + eps[i+1] # Julie: changed eps[i] to eps[i+1]
...
#eps = sigma*rnorm(nt)
#write(eps, file="epsilon_lake.txt", ncolumns=1) # write output to verify with Python
eps = scan("epsilon_lake.txt") # read in output to verify with Python

# Compute time series
  for(j in 1:nu)  { # loop over u
  u=uvec[j]
  simlist = Tsim(lamda,u,eps)
  Wsraw = simlist[[1]]
...
# write output to varify with Python
spectra = data.frame(sp.con[[2]], sp.nocon[[2]])
write.table(spectra, "lake_spectra.txt")
write.table(Wmat,"lakeP.txt")
