# Fish Harvest - 1D alt state model - Harvest mgt - spectra
# SRC 2014-05-14
...
Tsim = function(h,u,eps) {
  ...
  for( i in 3:(nt-1) )   {
    ...
    dx2=xt[i-1]-xt[i-2]
    gx[i]=Gfun(xt[i])
    # Julie: moved line below to after calculating xnext and made it depend on one time step later for everything
    #Ht[i+1] = -1*(- h*xt[i] + (u+v)*h*xt[i-1] - u*v*h*xt[i-2])
    #xnext=xt[i] + v*dx1 + gx[i] - v*gx[i-1] - h*xt[i] + (u+v)*h*xt[i-1] - u*v*h*xt[i-2] + eps[i] - w*eps[i-1]
    # Julie: changed above line to the following
    xnext=xt[i] + v*dx1 + gx[i] - v*gx[i-1] - Ht[i] + eps[i+1] - w*eps[i]
    xt[i+1] = max(xnext,0.1)
    Ht[i+1] = -1*(- h*xt[i+1] + (u+v)*h*xt[i] - u*v*h*xt[i-1])
  }
  outlist = list(Ht,xt)
  return(outlist)
}
...
# save output to file to test against Python code
harvest = data.frame(Hsim.con, Hsim.nocon)
write.table(harvest, "fishHarvest_harvest_mgmt.txt")
spectra = data.frame(sp.con[[2]], sp.nocon[[2]])
write.table(spectra, "fishSpectra_harvest_mgmt.txt")
